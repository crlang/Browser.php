 ...
 * @name: Browser.php
 * @Author: crlang
 * @Date: 2021-03-30 10:19:01
 * @Description: A PHP Class to detect a user's Browser. (https://github.com/crlang/Browser.php)
 * @Description-zh: 一个PHP类来检测用户的浏览器信息。
 * @LastEditors: crlang(https://www.crlang.com)
 * @LastEditTime: 2021-03-30 11:17:13
 * @version 2.0.0
 *
 * Typical Usage:
 *
...
class Browser
{
    ...
    {
        if (false !== stripos($this->_agent, 'Brave/')) {
        ...
        } elseif (false !== stripos($this->_agent, ' Brave ')) {
            $this->setBrowser(self::BROWSER_BRAVE);
    ...
    {
        if ($name = (false !== stripos($this->_agent, 'Edge/') ? 'Edge' : ((false !== stripos($this->_agent, 'Edg/') || false !== stripos($this->_agent, 'EdgA/')) ? 'Edg' : false))) {
    ...
    protected function checkBrowserInternetExplorer()
    {
        ...
        if (false !== stripos($this->_agent, 'microsoft internet explorer')) {
            $this->setBrowser(self::BROWSER_IE);
            $this->setVersion('1.0');
        ...
        if (false !== stripos($this->_agent, 'msie') && false === stripos($this->_agent, 'opera')) {
    ...
    protected function checkBrowserOpera()
    {
        ...
        if (false !== stripos($this->_agent, 'opera')) {
        ...
        if (false !== stripos($this->_agent, 'OPR')) {
    ...
    protected function checkBrowserNetscapeNavigator9Plus()
    {
        ...
        if (false === stripos($this->_agent, 'Firefox') && preg_match('/Netscape6?\/([^ ]*)/i', $this->_agent, $matches)) {
            $this->setVersion($matches[1]);
            $this->setBrowser(self::BROWSER_NETSCAPE_NAVIGATOR);
    ...
    protected function checkBrowserFirefox()
    {
        if (false === stripos($this->_agent, 'safari')) {
            if (preg_match('/Firefox[\\/ \\(]([^ ;\\)]+)/i', $this->_agent, $matches)) {
                ...
                //Firefox on Android
                if (false !== stripos($this->_agent, 'Android') || false !== stripos($this->_agent, 'iPhone')) {
                    if (false !== stripos($this->_agent, 'Mobile') || false !== stripos($this->_agent, 'Tablet')) {
                        $this->setMobile(true);
                    } else {
                        $this->setTablet(true);
                    }
    ...
    protected function checkBrowserMozilla()
    {
        ...
        if (false !== stripos($this->_agent, 'mozilla') && preg_match('/rv:[0-9]\.[0-9]/i', $this->_agent) && false === stripos($this->_agent, 'netscape')) {
            $aversion = explode('', stristr($this->_agent, 'rv:'));
            $this->setVersion(str_replace('rv:', '', $aversion[0]));
            $this->setBrowser(self::BROWSER_MOZILLA);
        ...
        if (false !== stripos($this->_agent, 'mozilla') && preg_match('/mozilla\/([^ ]*)/i', $this->_agent, $matches) && false === stripos($this->_agent, 'netscape')) {
            $this->setVersion($matches[1]);
            $this->setBrowser(self::BROWSER_MOZILLA);
    ...
    {
        if (
            false !== stripos($this->_agent, 'Safari')
